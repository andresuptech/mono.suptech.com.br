...
class FrontBoxController extends Controller
{
    ...
    /*public function filtro(Request $request){
       ...
       /* $dataInicial = $request->data_inicial;
		...
		if(isset($cliente) && isset($dataInicial) && isset($dataFinal) && isset($estado)){
		...
		}else if(isset($dataInicial) && isset($dataFinal) && isset($estado)){
			$vendas = VendaCaixa::filtroData(
				$this->parseDate($dataInicial),
				$this->parseDate($dataFinal, true),
				$estado
    ...
    public function filtro(Request $request){
		...
		->with('title', "Filtro de Vendas Caixa");*/
    ...
    public function relatorioVendasPDV(Request $request){
        $dataInicial = $request->data_inicial;
        $dataFinal = $request->data_final;
        $cliente = $request->cliente;
        $estado = $request->estado;
        ...
        return view('frontBox/relatorioVendasPDV')
        ->with('vendas', $vendas)
        ->with('cliente', $cliente)
        ->with('estado', $estado)
        ->with('dataInicial', $dataInicial)
